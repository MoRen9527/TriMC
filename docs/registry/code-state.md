...
- `src/config-sync/`：**NEW i4-2（init-collab-i4-five-dim-sync）**。五维同步接收侧：`types.ts`（bundle schema 契约 + 递归密钥字段拒绝 api_key/apiKey/secret/token + keys 白名单 + contentHash 五维语义哈希/指纹）、`status.ts`（applied/fleetHead/pending/dims 磁盘真源读取器 + mtime 轻缓存 D3）、`default-model.ts`（模型三级解析 env TRIMC_DEFAULT_MODEL > applied > 兜底常量 + flash 变体）、`apply.ts`（apply 执行体：读→校验→版本比对→员工维 sourceCommit 拉齐→落地→退出码）。装配点：`src/server/app.ts`（`GET /internal/v1/config/sync/status` + 模型名 4 处三级解析）、`src/pipeline/assemble.ts`、`src/agent-loop/sub-agent/spawn.ts`（模型 default 收敛 6 处全替换，旧名 deepseek-v4-pro/flash 仅存于 default-model.ts 兜底常量位）、`src/config/env.ts`（defaultModel/TRIMC_DEFAULT_MODEL 条目）。38 用例全绿，全量 541/540 PASS（基线 502/503 不退化）。
