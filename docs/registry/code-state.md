...
- `src/cli.ts`：**NEW r1-2**。`trimc cron` 子命令 CLI（add/list/run/log/status/update/remove + `--plane-shift` 预设），HTTP 客户端形态对标 trilc cron；package.json `bin.trimc = ./dist/src/cli.js`。
